...
[source,bash]
----
kubectl get gateway -n $GATEWAY_NAMESPACE $GATEWAY_NAME -ojson | jq '.metadata.annotations["airlock-microgateway.appuio.io/httproute-default-cluster-issuer"] or error("Gateway has no default issuer. Annotate the HTTPRoute with a `airlock-microgateway.appuio.io/cluster-issuer` or `airlock-microgateway.appuio.io/issuer` annotation or add the `airlock-microgateway.appuio.io/httproute-default-cluster-issuer` annotation to the Gateway.")'
----
