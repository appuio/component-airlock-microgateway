...
local certForHTTRoute(route) =
  ...
  if getAnnotation(route, createCertificateAnnotation) != 'true' then
    ok(null)
  else if secretName == '' then
    err('tls secret name annotation %(annotation)s not set on HTTPRoute %(namespace)s/%(name)s' % {
      annotation: tlsSecretNameAnnotation,
